...
export class InstructorProfilePageComponent implements OnInit {
  ...
  deleteSelectedCourse(course_delete:number){
    for(let x of this.courses)
    {
      if(course_delete==x.courseid)
      {
        this.iProfileService.deleteCourse(course_delete).subscribe( (data : string) => {
          alert(data);
        });
      }
    }
